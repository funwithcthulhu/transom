...
let run_doctor () =
  ...
  in
  let statuses =
    List.map (fun (name, required) -> name, required, command_exists name) tools
  in
  List.iter
    (fun (name, _, ok) ->
  ...
  match
    List.filter_map
      (fun (name, required, ok) -> if required && not ok then Some name else None)
      statuses
  with
  | [] -> Ok ()
  | missing ->
    result_error ("missing required OCaml tools: " ^ String.concat ", " missing)
...
let rec mkdir_p dir =
  if dir = "" || dir = Filename.dirname dir then
    ...
    | Ok () ->
      (try Ok (Unix.mkdir dir 0o755) with Unix.Unix_error (err, _, _) ->
...
let replace_all text ~pattern ~replacement =
  ...
  let rec loop index =
    if index > text_len - pattern_len then
      ...
      loop (index + 1))
  in
  if pattern_len = 0 then text else (loop 0; Buffer.contents buffer)
...
let is_text_file path =
  ...
  | "dune" | "dune-project" | "package.json" | "Cargo.toml" | "tauri.conf.json" -> true
  | _ ->
    List.mem
      (extension path)
...
let copy_file ~replacements ~src ~dst =
  try
    ...
    let content =
      if is_text_file src then replace_placeholders content replacements else content
    in
  ...
  | name :: rest ->
    ...
    let result =
      if Sys.is_directory src_path then
        ...
        copy_file ~replacements ~src:src_path ~dst:dst_path
    in
    (match result with
...
let capitalize_ascii word =
  ...
  | word ->
    String.mapi
      (fun index char -> if index = 0 then Char.uppercase_ascii char else char)
      word
...
let target_is_available ~force target =
  if not (Sys.file_exists target) then
    ...
    Ok ()
  else if Sys.readdir target = [||] then
    Ok ()
  else
    result_error (target ^ " already exists; use --force to overwrite files")
...
let find_template template =
  let rec loop = function
    ...
    | dir :: rest ->
      ...
      if Sys.file_exists path && Sys.is_directory path then Some path else loop rest
  in
...
let cmd name doc term =
  Cmd.v (Cmd.info name ~doc) (term_result term)
...
let doctor_cmd =
  cmd "doctor" "Check for common local tools." Term.(const run_doctor $ const ())

let check_cmd =
  cmd
    "check"
    "Validate a Transom manifest."
    Term.(const run_check $ manifest_arg)

let gen_cmd =
  cmd
    "gen"
    "Generate OCaml server and TypeScript client glue."
